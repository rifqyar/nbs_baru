...
class PrayaGuzzleClient
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->client = new \GuzzleHttp\Client(array_merge($defaultConfig, $config));
    }
...
if (!function_exists('sendDataFromUrl')) {
    function sendDataFromUrl($payload_request, $url, $method = "POST", $token = "")
    {
        ...
        if (str_contains(strtolower($msg['msg']), 'authorization expired') || str_contains(strtolower($msg['msg']), 'jwt expired')) {
            // Token expired, re-login
    ...
    function sendDataFromUrlNode($payload_request, $url, $method = "POST", $token = "")
    {
        $payload = [
    ...
    function save_payment_uster($payload)
    {
        try {
            ...
            if ($jenis == 'STRIPPING' || $jenis == 'PERP_STRIP') { //DELIVERY KALO DARI SISI TPK
                ...
                $eta = $get_vessel['eta'] ?? null;
                $etb = $get_vessel['etb'] ?? null;
                $etd = $get_vessel['etd'] ?? null;
                $ata = $get_vessel['ata'] ?? null;
                $atb = $get_vessel['atb'] ?? null;
                $atd = $get_vessel['atd'] ?? null;
                $startWork = $get_vessel['start_work'] ?? null;
                $endWork = $get_vessel['end_work'] ?? null;
                ...
                foreach ($fetchContainerStripping as $k => $v) {
                    ...
                    array_push(
                        ...
                        array(
                            ...
                            "containerDischargeDate" => $get_vessel['discharge_date'] ?? null,
                            "disabled" => "Y"
                        )
                    );
                }
