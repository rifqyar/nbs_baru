...
class VoyController extends Controller
{
    ...
    function index()
    {
        return view('report.container.area.voy');
    }

    function dataTables(Request $request): JsonResponse
    {

        $listDelivery = $this->report->DataTableArea($request);
        return DataTables::of($listDelivery)->make(true);
    }

    // function nota(Request $request)
    // {
    //     $viewData = $this->report->notaRequest($request->term);
    ...
    function generateExcel(Request $request)
    {
        ...
        return view('report.container.area.toexcel.toexcel', $data);
    }
