...
class NotaStuffingExt
{
    ...
    function PrintProforma($no_req)
    {
        ...
        $data = DB::connection('uster_dev')->table(DB::raw('nota_stuffing@DBCLOUD_LINK as a'))
            ->join(DB::raw('request_stuffing@DBCLOUD_LINK as c'), 'a.NO_REQUEST', '=', 'c.NO_REQUEST')
            ->leftJoin(DB::raw('BILLING_NBS.TB_USER@DBCLOUD_LINK as mu'), 'a.nipp_user', '=', 'mu.id')
