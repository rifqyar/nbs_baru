    ...
    function DataTable($request)
    {
        ...
        $query = DB::connection('uster')
            ...
            ->select([
                ...
                DB::raw("NVL((SELECT username FROM master_user WHERE TO_CHAR(id) = TO_CHAR(c.id_user)), c.id_user) as username"),
        ...
        // Status filter
        if ($status) {
            if (in_array($status, ['FCL', 'MTY', 'LCL'])) {
                $query->where('b.status', $status);
            }
        }

        $query->orderByDesc('c.tgl_in');
        return $query->get();
    }
