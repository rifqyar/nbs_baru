...
class DeliveryService
{
    ...
    public function dataDeliveryLuar($request)
    {
        ...
        if (isset($from) || isset($to) || isset($no_req)) {
            ...
            //                           b.NM_PBM AS NAMA_EMKL, count(c.no_container) jumlah, d.LUNAS
            //                     FROM REQUEST_DELIVERY a,
            //                             container_delivery c,
            //                          v_mst_pbm b
            //                     LEFT JOIN NOTA_DELIVERY d ON REQUEST_DELIVERY.NO_REQUEST = d.NO_REQUEST
            //                     WHERE a.DELIVERY_KE = 'LUAR'
            //                     and a.no_request = c.no_request
            //                     AND a.KD_EMKL = b.KD_PBM
            //                     AND b.KD_CABANG = '05'
            ...
            $query_list = "SELECT *
                                FROM (
                                    ...
                                    LEFT JOIN NOTA_DELIVERY d ON a.NO_REQUEST = d.NO_REQUEST
                                    WHERE
                                        a.DELIVERY_KE = 'LUAR'
                                        AND a.perp_dari IS NULL
                                        AND a.PERALIHAN NOT IN ('RELOKASI', 'STUFFING', 'STRIPPING')
    ...
    function getNoContainer($no_cont)
    {
        ...
        $query             = "SELECT DISTINCT a.NO_CONTAINER,
                   ...
                   -- AND B.STATUS_CONT = 'MTY'
                    and b.kegiatan IN ('REALISASI STRIPPING','GATE IN','BATAL STUFFING','BATAL RECEIVING','REQUEST BATALMUAT','REQUEST DELIVERY', 'BORDER GATE IN', 'BATAL STRIPPING', 'PERPANJANGAN STRIPPING', 'REALISASI STUFFING', 'BATAL SP2')
					--UNION
